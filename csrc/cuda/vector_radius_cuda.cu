...
__global__ void
vector_radius_kernel(const scalar_t *__restrict__ x, const scalar_t *__restrict__ y,
  ...
  for (int64_t n_x = ptr_x[example_idx]; n_x < ptr_x[example_idx + 1]; n_x++) {
    ...
    for (int64_t d = 0; d < dim; d++) {
      dot += (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d]) *
             (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d]);
    }
    if(dot <= 0.0) {
      for (int64_t d = 0; d < dim; d++) {
        dist += (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d]) *
               (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d]);
      }
    ...
    // Check for V2 bound
      ...
      for (int64_t d = 0; d < dim; d++) {
        dot += (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d]) *
               (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d + 1]);
      }
      if(dot >= 0.0) {
        for (int64_t d = 0; d < dim; d++) {
          dist += (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d + 1]) *
                  (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d + 1]);
        }

      // Otherwise calculate distance to the line
      } else {
        scalar_t norm = 0;
        scalar_t scalar = 0;
        ...
        for (int64_t d = 0; d < dim; d++) {
          dot += (x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d]) *
                 (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d]);

          norm += (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d]) *
                  (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d]);
        }
        ...
        for (int64_t d = 0; d < dim; d++) {
            dist += (
                (
                    x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d] - 
                    scalar * (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d])
                ) *
                (
                    x[n_x * dim + d] - y[n_y * 2 * dim + 2 * d] - 
                    scalar * (y[n_y * 2 * dim + 2 * d + 1] - y[n_y * 2 * dim + 2 * d])
...
torch::Tensor vector_radius_cuda(const torch::Tensor x, const torch::Tensor y,
  ...
  CHECK_CONTIGUOUS(y);
  CHECK_INPUT(y.dim() == 2);
  CHECK_INPUT(2 * x.size(1) ==  y.size(1));
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(at::ScalarType::Half, scalar_type, "_", [&] {
    vector_radius_kernel<scalar_t><<<BLOCKS, THREADS, 0, stream>>>(
